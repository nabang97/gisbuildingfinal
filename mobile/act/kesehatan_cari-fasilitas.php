...
$querysearch="	SELECT H.health_building_id, H.name_of_health_building, ST_X(ST_Centroid(H.geom)) AS lng, ST_Y(ST_CENTROID(H.geom)) AS lat FROM health_building AS H	JOIN detail_health_building_facilities AS F on H.health_building_id=F.health_building_id	WHERE F.facility_id IN ($f) GROUP BY F.health_building_id, H.health_building_id, H.name_of_health_building HAVING COUNT(*) = '$total'";
